	...
	)
{
	...
	eReturn = main_NtQueueApcThreadWrapper(
		...
		hWaitHandle, //pvArg1 for pfnWaitForSingleObjectEx()
...
ESTATUS main_EnumProcessThreadIds(
	...
	)//Skip those threads with owner's PID != currentprocess
{
	HANDLE hSnapshot = NULL;
	ESTATUS eReturn = ESTATUS_INVALID;
	...
	)
{
	...
	eReturn = main_EnumProcessThreadIds(
		...
		);//look for the threads that has the target process as its owner
	if (ESTATUS_FAILED(eReturn))
	{
		goto lblCleanup;
	}
...
ESTATUS main_FindAlertableThread(HANDLE hProcess, PHANDLE phAlertableThread)
{
	...
	eReturn = main_EnumProcessThreads(
		...
		&dwNumberOfProcessThreads
		);//Retrieve Handles to Threads that is owned by current process
	if (ESTATUS_FAILED(eReturn))
	{
		goto lblCleanup;
	}

	//Go to every thread owned by target process and queue an APC routine that call WaitForSingleObjectEx() and for 5000ms / current thread to terminate.
	for (DWORD dwIndex = 0; dwIndex < dwNumberOfProcessThreads; dwIndex++)
	{
	...
	for (DWORD dwIndex = 0; dwIndex < dwNumberOfProcessThreads; dwIndex++)
	{
		...
		phLocalEvents[dwIndex] = CreateEvent(NULL, TRUE, FALSE, NULL); //(Default security attrib,manual reset event,init state = signaled,unnamed event obj
		...
		bErr = DuplicateHandle(
			...
			0,//dwDesiredAccess is ignored due to DUPLICATE_SAME_ACCESS
			FALSE,//bInheritHandle, If FALSE, the new handle cannot be inherited.
		...
		//set APC in target thread with routine SetEvent() with event handle dup from current thread
		//SetEvent() Sets the specified event object to the signaled state
		//To force the target thread to queue the APC, the target thread is suspended, APC is quweu
